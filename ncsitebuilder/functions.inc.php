		...
		foreach($fields as $fKey => $field) {
			...
			if( $field["type"] === "file" ) {
				...
				if (empty($post[$fieldName]) || count($field['settings']['options']) == 1) {
					$fields[$fKey]['name'] = $fields[$fKey]['name'] ? $fields[$fKey]['name'] : $field['settings']['options'][0];
				...
				if (empty($valueRaw) && strlen($valueRaw) == 0 && $required) {
					...
					if ($field["type"] == "select") {
						...
						$data[$idx] = trim(in_array($value, $options) ? $value : (isset($options[intval($value)]) ? $options[intval($value)] : ''));
					} else
						$data[$idx] = $value;
				}
