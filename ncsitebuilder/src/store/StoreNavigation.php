...
class StoreNavigation {
	...
	public function getUri($route = '') {
		...
		$path = substr($path, 0, strpos($route, '?') ?: strlen($route));
		$hasTrailingSlash = (substr(ltrim($path, '/'), -1) == '/');
		if (!$hasTrailingSlash && !!SiteModule::$siteInfo->useTrailingSlashes) {
			$route = $path . '/' . substr($route, strlen($path));
		}

		$defLang = $this->defLang ? $this->defLang : $this->baseLang;
		$uri = $this->baseUrl.(($this->lang == $defLang || !$this->lang) ? '' : ($this->lang.'/')).ltrim($route, '/');
		return $uri;
	}
	...
	public function detailsUrl($item = null, $category = null, $noCurrent = false, $params = null, $qsa = false) {
		$catPart = ($category
				? (is_string($category) ? $category : ((tr_($category->alias) ? tr_($category->alias) : 'store-cat-'.$category->id)))
