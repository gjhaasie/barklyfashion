		...
		<tr>
			...
			<td width="50%">
				...
				<?php
					...
					foreach ($order->getBillingForm() as $idx => $field) {
						$key = strpos($field['type'], 'wb_store_') === 0 ? substr($field['type'], strlen('wb_store_')) : ('wb_input_' . $idx);
						$value = isset($billingInfo->$key) ? $billingInfo->$key : null;
						if (empty($value)) continue;
						if (($field = StorePaymentApi::getBillingFieldByKey($key)) && $field->type === 'file') continue;
						$fieldName = StorePaymentApi::getInfoHtmlTableFieldName($key);
						$fieldValue = StorePaymentApi::getInfoHtmlTableFieldValue($value, $key);
						if ($fieldName === null) continue;
						echo StoreInvoice::fontize($fieldName) . ': ' . StoreInvoice::fontize($fieldValue).'<br/>';
					}
				?>
			</td>
		</tr>
